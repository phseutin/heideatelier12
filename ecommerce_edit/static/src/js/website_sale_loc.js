...
odoo.define('website_sale.cart', function (require) {
    ...
    sAnimations.registry.WebsiteSale = sAnimations.Class.extend(ProductConfiguratorMixin, {
        ...
        read_events: {
            ...
            'change select[name="location_id"]': '_onChangeLocation',
        ...
        init: function () {
            ...
            this._changeLocation = _.debounce(this._changeLocation.bind(this), 500);
        ...
        _changeLocation: function () {
            ...
            this._rpc({
                route: "/shop/selection_infos/" + $("#location_id").val(),
                params: {
                    mode: 'shipping',
                },
            }).then(function (data) {
                ...
                // populate states and display
                var selectDate = $("select[name='date_id']");
                // dont reload state at first loading (done in qweb)
                if (selectDate.data('init')===0) {
                    if (data.date_ids.length) {
                        selectDate.html('');
                        _.each(data.date_ids, function (x) {
                            var opt = $('<option>').text(x[1])
                                .attr('value', x[0])
                                .attr('data-name', x[1]);
